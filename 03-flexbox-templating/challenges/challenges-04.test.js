...
const isCapitalized = (str) => {
  const firstLetter = /[A-Z][a-zA-z]*/g
  return str.match(firstLetter)
};
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  const firstLetter = /^[A-J]/
  const cityArray = []
  arr.forEach((city) => {
    if(firstLetter.test(city)) cityArray.push(city)
  })
  return cityArray
};
...
const noPunctuation = str => {
  // Solution code here...
  const endSpace = /\w+\s/g
  return str.match(endSpace)
};
